...
- (BOOL)shouldPerformSegueWithIdentifier:(NSString *)identifier sender:(id)sender
{
    ...
    if ( [identifier isEqualToString:kWTSegueIdentifier_SaveURL] )
    {
        ...
        if ( nil == experience )
        {
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Invalid information" message:@"Please verify the URL.\nIs it ATS compliant?" delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
            [alert show];
        }
    }
...
- (WTAugmentedRealityExperience *)experienceFromCurrentInput
{
    ...
    WTFeatures features = [self featuresFromSwitchStates];
    NSDictionary *startupParameters = [self startupParametersFromSwitchStates];
    ...
    {
        return [WTAugmentedRealityExperience experienceWithTitle:title groupTitle:[URL host] URL:URL requiredFeatures:features startupParameters:startupParameters requiredViewControllerExtension:nil];
    }
}

#pragma mark Augmented Reality Experience Creation
- (WTFeatures)featuresFromSwitchStates
{
    WTFeatures features;

    if ( self.requiredFeature2dtrackingSwitch.on )
    {
    ...
    if ( self.requiredFeatureGeoSwitch.on )
    {
...
- (NSDictionary *)startupParametersFromSwitchStates
{
    NSString *captureDeviceSwitchState = self.captureDevicePositionSwitch.on ? @"front" : @"back";
    NSDictionary *startupParameters = @{kWTAugmentedRealityExperienceStartupParameterKey_CaptureDevicePosition: captureDeviceSwitchState};
...
{
    [self preFillWithPrefix:@"https://goo.gl/"];
}

- (void)preFillWithTinyURL
{
    [self preFillWithPrefix:@"https://tinyurl.com/"];
}
